...
const AddSeries = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/categories/all-categories`, { method : 'GET' , credentials : 'include' });
    ...
    const uploadToCloud = async (file, single = true) => {
        ...
        if (single) {
            ...
            const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/upload/uploadFile`, { method: 'POST', body: formData, credentials: 'include' });
            const data = await res.json();
            return data?.data?.secure_url;
        } else {
            file.forEach(f => formData.append('MultipleImages', f));
            const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/upload/uploadFiles`, { method: 'POST', body: formData, credentials: 'include' });
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/series/add-series`, {
                method: 'POST',
                headers: {
